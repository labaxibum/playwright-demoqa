...
export class bookApiHelper {
    static async createAccountAPI(request, username, password) {
        const responseCreateAccount = await request.post(BASE_URL + "Account/v1/User", {
            data: {
                "userName": username,
                "password": password
            }
    ...
    static async getTokenAPI(request, username, password) {
        const responseToken = await request.post(BASE_URL + "Account/v1/GenerateToken", {
            data: {
                "userName": username,
                "password": password
            }
    ...
    static async authorizationAPI(request, username, password) {
        const responseAutho = await request.post(BASE_URL + "Account/v1/Authorized", {
            data: {
                "userName": username,
                "password": password
            }
    ...
    static async addBookAPI(request, token, userID, isbn) {
        const responseAddBook = await request.post(BASE_URL + "BookStore/v1/Books", {
            headers: {
    ...
    static async deleteBookAPI(request, token, isbn, userID) {
        const reponseDeleteBooks = await request.delete(BASE_URL + "BookStore/v1/Book", {
            headers: {
                'Accept': 'application/json',
                'Authorization': `Bearer ${token}`
            },
